...
pub struct Fonts {
    pub ft: FT_Library,
    pub fonts: ManuallyDrop<HashMap<FontID, Font>>,
}

#[derive(Debug)]
pub struct Font {
    pub face: FT_Face,
    pub texture: grx::Texture2D,
...
impl From<FontID> for grx::TextureUnit {
    fn from(f: FontID) -> Self {
        match f {
            FontID::Debug => grx::TextureUnit::DebugFontAtlas,
            FontID::Normal => grx::TextureUnit::NormalFontAtlas,
        }
    }
}
...
impl FontID {
    pub fn try_from_texture_unit(u: grx::TextureUnit) -> Option<Self> {
        match u {
            grx::TextureUnit::DebugFontAtlas => Some(FontID::Debug),
            grx::TextureUnit::NormalFontAtlas => Some(FontID::Normal),
        }
    }
}
...
impl Font {
    ...
    ) -> Result<Self, String> 
    {
        ...
        unsafe {
            gl::ActiveTexture(texture_unit.to_gl());
        }
        let image = grx::Texture2DImage::from_greyscale_u8(&pixels, tex_size);
        let texture = grx::Texture2D::new(grx::Texture2DInit {
            image, 
            sampler_params: grx::SamplerParams::new_clamp_to_edge_linear(),
...
impl Fonts {
    pub fn from_path(path: &Path) -> Result<Self, String> {
        ...
        let fonts = {
            ...
            let petita = load_font("petita", "PetitaMedium.ttf", 22, grx::TextureUnit::NormalFontAtlas, 256)?;
            let mut fonts = HashMap::new();
            fonts.insert(FontID::Debug, basis33);
            fonts.insert(FontID::Normal, petita);
            fonts
        };
